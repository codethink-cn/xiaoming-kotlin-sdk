...
import cn.codethink.xiaoming.plugin.jvm.LocalJvmPluginHandler

/**
 * 本地 JVM 经典插件：一个 [LocalJvmClassicPluginHandler] 的非抽象子类，成为**插件主类**。
 ...
 * 随后使用 [LocalJvmClassicPluginClassPath.pluginClassLoader] 加载此类。
 *
 * 在同一个文件夹下，还可以存在 `access.yml` 文件，以声明插件类的访问权限。
 *
 * @author Chuanwise
 */
interface LocalJvmClassicPluginHandler : LocalJvmPluginHandler {
