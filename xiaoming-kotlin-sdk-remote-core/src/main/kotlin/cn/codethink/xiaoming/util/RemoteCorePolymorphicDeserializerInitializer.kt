...
class RemoteCorePolymorphicDeserializerInitializer : PolymorphicDeserializerInitializer {
    override fun initialize(deserializers: MutablePolymorphicDeserializers, subject: SubjectDescriptor) {
        deserializers.subject(subject) {
            names<Packet>(FIELD_TYPE) {
                type<RequestPacket, RequestPacketImpl>()
                type<ReceiptPacket, ReceiptPacketImpl>()
