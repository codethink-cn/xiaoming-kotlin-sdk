...
import cn.codethink.xiaoming.util.rootCause
...
class WebSocketClientConnectionApi(
...
) : WebSocketConnectionApi, CoroutineScope {
    private val job = SupervisorJob(parentJob)
    private val scope: CoroutineScope = CoroutineScope(parentCoroutineContext + job)
    override val coroutineContext: CoroutineContext by scope::coroutineContext

    override val channel: Channel<Received<Frame>> = Channel(Channel.UNLIMITED)
    ...
    override suspend fun receive(data: Frame, origin: Any?) {
        ...
        channel.send(createReceived(origin, data))
        logger.debug { "Received: '$data' from ${configuration.address}." }
    }

    override fun close(): Unit = runBlocking {
        close(createCause("Client closed.", descriptor), descriptor)
    }
